...
const getPusher = () => {
    if (!pusherInstance) {
        pusherInstance = new Pusher(process.env.NEXT_PUBLIC_PUSHER_KEY || "app-key", {
            ...
            cluster: "mt1" // dummy cluster if keys require it
        });
    }
...
export function useRealtime(config: any) {
    ...
    configRef.current = config; // Always keep the latest callbacks without reconnecting
        ...
        return () => {
            channelsToSub.forEach(ch => {
                ...
                if (channel) {
                    ...
                    // Only unsubscribe if there are no more bindings
                    // Actually, let's keep it subscribed for page lifetime for simplicity,
                    // or pusher unbinds specific callbacks
                }
            });
        };
        // Re-run effect only if the channel string itself changes, to prevent socket churn
